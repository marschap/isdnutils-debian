...
int new_client(int sock)
{
	...
	if (Old_Info != NULL)
		for (Cnt = 0; Cnt < MAX_CALLS_IN_QUEUE; Cnt++)
	...
	if (Old_Prints != NULL && User_Get_Message(sockets[sock].f_username,sockets[sock].f_hostname,NULL,T_PROTOCOL) == 0)
		for (Cnt = 0; Cnt < MAX_PRINTS_IN_QUEUE; Cnt++)
...
*/
      ...
      sprintf(RetCode, "%d|%d|%d|%s|%s|%s|%s|%s|%s|%s|%d|%d|%d|%s|%s|%ld|%ld|%g|%g|%s\n",
        ...
        double2str(abs(call->aoce) * (currency_factor ? currency_factor : einheit), 6, 2, DEB), /* kostet gerade */
...
int append_Old_Prints(char *String)
{
	...
	if (Old_Prints == NULL)
		if ((Old_Prints = (char**) calloc(MAX_PRINTS_IN_QUEUE,sizeof(char*))) == NULL)
			return NO_MEMORY;


	while(Cnt < MAX_PRINTS_IN_QUEUE && Old_Prints[Cnt] != NULL)
		Cnt++;

	if (Cnt == MAX_PRINTS_IN_QUEUE)
	{
		Cnt = 1;
		...
		while(Cnt < MAX_PRINTS_IN_QUEUE)
		{
			Old_Prints[Cnt-1] = Old_Prints[Cnt];
			Cnt++;
		}

		Cnt = MAX_PRINTS_IN_QUEUE - 1;
	}
...
int append_Old_Info(char *String)
{
	...
	if (Old_Info == NULL)
		if ((Old_Info = (char**) calloc(MAX_CALLS_IN_QUEUE,sizeof(char*))) == NULL)
			return NO_MEMORY;


	while(Cnt < MAX_CALLS_IN_QUEUE && Old_Info[Cnt] != NULL)
		Cnt++;

	if (Cnt == MAX_CALLS_IN_QUEUE)
	{
		Cnt = 1;
		free(Old_Info[0]);

		while(Cnt < MAX_CALLS_IN_QUEUE)
		{
			Old_Info[Cnt-1] = Old_Info[Cnt];
			Cnt++;
		}

		Cnt = MAX_CALLS_IN_QUEUE - 1;
	}
