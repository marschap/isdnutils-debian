...
static char *Kill_Blanks(char *String);
static int Set_Flags(access_flags **Cursor, char *String);
static int Set_Entry (char* Name, char* User, char *Host, char *Flags);
...
int read_file (void)
{
	...
	while (FGets(String, BUFSIZ, fp, &Line) != NULL)
	{
		...
		if ((RetCode = Set_Entry(Name,User,Host,Flags)) != 0)
			return RetCode;
	}
...
{
	int Cnt1 = 0, Cnt2 = 0;
	char Help[LONG_STRING_SIZE] = "";


	while (String[Cnt1] != '\0')
	{
		if (String[Cnt1] != ' ' && String[Cnt1] != '\t')
			Help[Cnt2++] = String[Cnt1];
...
static int create_new_file(void)
{
	...
	Set_Entry("isdnlog-local","","","");
	Set_Entry("","","localhost","");

	setpwent();
	while ((password = getpwent()) != NULL)
	{
		Set_Entry("",password->pw_name,"",Name);
	}
	endpwent();
			...
			if (mymsn != NULL && Ptr->type == FLAG_MSN)
			{
				...
				if (Options)
					while(Options[Cnt] != NULL)
					{
						...
						if (match(mymsn,Help) || (*mymsn == '\0' && !strcmp(Options[Cnt],"*")))
							return 0;
